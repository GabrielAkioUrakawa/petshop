...
let ServicoRepository = class ServicoRepository {
    ...
    async findByFornecedor(nomeFornecedor) {
        const result = await this.pool.query(`SELECT F.NOME AS NOME_FORNECEDOR, P.DESCRICAO AS NOME_PRODUTO, S.TIPO AS TIPO_SERVICO, S.DATA_HORA
       FROM FORNECEDOR F
    ...
    async findByDate(dataEspecifica) {
       ...
       FROM SERVICO S
       JOIN REALIZA R ON S.SERVICO_CPF = R.SERVICO_CPF AND S.DATA_HORA = R.SERVICO_DATA_HORA
